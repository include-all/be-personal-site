...
const getHupuTop = async ({
  ...
  try {
    ...
    postWrap.each((index, node) => {
      ...
      let $postDatum = $(node).find('.post-datum')
      res.push({
        postName: $post.text().trim(),
        postHref: $post.attr('href'),
        replyCount: $postDatum.text().split('/')[0].trim(),
        type,
      })
    })
    await hupuPostModel.updatePost(res)
  } catch (err) {
    console.log(err)
  }
}
